<script>
  ...
  async function openDetail(employee) {
    selected = employee; detailLoading = true; showMarks = false; error = ''
    try {
      ...
      marcaciones = list(attendanceData)
        .filter((item) => String(item.empleadoId) === String(employee.id) && item.fechaHoraReal && new Date(item.fechaHoraReal) >= since)
  ...
  async function resolveApproval(employee, approve) {
    ...
    } catch (cause) { handleError(cause, 'No se pudo actualizar la aprobación.') }
  ...
  async function createEmployee(event) {
    ...
    if (!form.nombreCompleto.trim() || !form.correo.trim() || !form.documentoIdentidad.trim() || (!editingId && !form.password)) { formError = 'Completá todos los campos obligatorios.'; return }
    saving = true
...
<section class="view-stack">
  ...
  <div class="panel table-wrap">
    <table>
      <thead><tr><th>Empleado</th><th>Documento</th><th>Rol</th><th>Aprobación móvil</th><th>Estado</th>{#if canManageEmployees(role)}<th>Acciones</th>{/if}</tr></thead>
      <tbody>
        {#if loading}
  ...
  {#if selected}
    <section class="detail-panel">
      <div class="detail-header"><div><span class="eyebrow">Ficha de empleado</span><h2>{selected.nombreCompleto}</h2><p>{selected.activo ? 'Empleado activo' : 'Empleado inactivo'} · aprobación {selected.estadoAprobacion || 'PENDIENTE'}</p></div><div class="row-actions"><button class="primary-button" type="button" onclick={() => showMarks = !showMarks}>{showMarks ? 'Ocultar marcaciones' : 'Ver marcaciones'}</button><button class="secondary-button" type="button" onclick={() => selected = null}>Cerrar</button></div></div>
      ...
      <div class="detail-section"><h3>Obras asignadas</h3>{#if detailLoading}<p class="table-message">Cargando ficha…</p>{:else if !asignaciones.length}<p class="empty-inline">Este empleado todavía no tiene obras asignadas.</p>{:else}<div class="table-wrap"><table><thead><tr><th>Código SAP</th><th>Obra</th><th>Desde</th><th>Entrada</th><th>Salida</th></tr></thead><tbody>{#each asignaciones as assignment}<tr><td><strong>{assignment.obraId || '—'}</strong></td><td>{assignment.obraNombre || '—'}</td><td>{formatDate(assignment.fechaInicio)}</td><td>{assignment.horaEntrada || '08:00'}</td><td>{assignment.horaSalida || '17:00'}</td></tr>{/each}</tbody></table></div>{/if}</div>
      {#if showMarks}<div class="detail-section"><h3>Marcaciones de los últimos 30 días</h3>{#if detailLoading}<p class="table-message">Cargando marcaciones…</p>{:else if !marcaciones.length}<p class="empty-inline">Sin marcaciones durante este período.</p>{:else}<div class="table-wrap"><table><thead><tr><th>Fecha y hora</th><th>Tipo</th><th>Obra</th><th>Estado</th><th>Motivo</th><th>Dispositivo</th></tr></thead><tbody>{#each marcaciones as mark}<tr><td>{formatDateTime(mark.fechaHoraReal)}</td><td><span class="pill neutral">{mark.tipoMarcacion || 'ENTRADA'}</span></td><td>{mark.obraId || '—'} · {mark.obraNombre || '—'}</td><td><span class="pill {mark.requiereRevision ? 'pendiente' : 'aprobado'}">{mark.requiereRevision ? 'Revisión' : 'Conforme'}</span></td><td>{mark.motivoRevision || '—'}</td><td>{mark.deviceId || '—'}</td></tr>{/each}</tbody></table></div>{/if}</div>{/if}
    </section>
  {/if}
</section>

{#if showCreate}<div class="modal-backdrop" role="presentation" onclick={(event) => event.target === event.currentTarget && (showCreate = false)}><div class="modal-card" role="dialog" aria-modal="true" aria-labelledby="employee-create-title"><div class="detail-header"><div><span class="eyebrow">Personal</span><h2 id="employee-create-title">{editingId ? 'Editar empleado' : 'Nuevo empleado'}</h2></div><button class="icon-button" aria-label="Cerrar" onclick={() => showCreate = false}>×</button></div><form class="modal-form" onsubmit={createEmployee}><label>Nombre completo<input bind:value={form.nombreCompleto} autocomplete="name" /></label><label>Correo electrónico<input type="email" bind:value={form.correo} autocomplete="email" /></label><label>Documento de identidad<input bind:value={form.documentoIdentidad} /></label><div class="form-grid"><label>Rol<select bind:value={form.rol}><option value="OPERATIVO">Operativo</option><option value="RRHH">RRHH</option><option value="ADMIN">Administrador</option><option value="JEFE_OBRA">Jefe de obra</option><option value="RESIDENTE">Residente</option>{#if role === 'SOPORTE'}<option value="SOPORTE">Soporte</option>{/if}</select></label><label>Tipo de contrato<select bind:value={form.tipoContrato}><option value="INTERNO">Interno</option><option value="TERCERIZADO">Tercerizado</option></select></label></div><label>{editingId ? 'Nueva contraseña (opcional)' : 'Contraseña inicial'}<input type="password" bind:value={form.password} autocomplete="new-password" /></label>{#if formError}<div class="alert error">{formError}</div>{/if}<div class="modal-actions"><button class="secondary-button" type="button" onclick={() => showCreate = false}>Cancelar</button><button class="primary-button" type="submit" disabled={saving}>{saving ? 'Guardando…' : (editingId ? 'Guardar cambios' : 'Crear empleado')}</button></div></form></div></div>{/if}
